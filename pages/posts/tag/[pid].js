...
class Home extends React.Component {
  ...
  static getInitialProps = async ({ query }) => {
    const tagId = query.pid;
    try {
      const tagRes = await axios.get(`/api/tags/${tagId}`);
      const postsRes = await axios.get(`/api/posts?status=1&tag=${tagId}`);
  ...
  handleFetchPostsInPage = async page => {
    await this.props.readPosts(page, null, this.props.tag._id);
    this.setState({
  ...
  render() {
    ...
    const { categories, tags, tag } = this.props;
